...
namespace Offsets {
	...
	DWORD Method_from_interpreted_entry_offset = NULL;

	DWORD nmethod_verified_entry_point_offset = NULL;

	DWORD integrity_check_offset = NULL;
...
#define METHOD_FROM_INTERPRETED_ENTRY_PATTERN "49 8B 84 24 ? ? ? ? 49 8B CF 48 89 45 B8"

#define NMETHOD_VERIFIED_ENTRY_POINT_PATTERN "48 03 C7 48 89 87 ? ? ? ? 48 89 B7"

#define INTEGRITY_CHECK_PATTERN "49 8B 41 10 0F B6 10"
...
#define FIND_OFFSET(pattern, result, module_info) {														\
	std::vector<BYTE*> findings;																		\
	AOBScanner::Scan(																					\
		process,																						\
		pattern,																						\
		findings,																						\
		AOBScanner::RegionAttributes(PAGE_EXECUTE_WRITECOPY, MEM_COMMIT, PAGE_EXECUTE_READ, MEM_IMAGE),	\
		reinterpret_cast<BYTE*>(module_info.lpBaseOfDll),												\
		reinterpret_cast<BYTE*>(module_info.lpBaseOfDll) + module_info.SizeOfImage						\
	);																									\
																										\
	if (findings.size() == 1) result = findings[0] - (BYTE*)module_info.lpBaseOfDll;					\
}																										\
	
	void InitializeIntegrityChecks() {
		HANDLE process = GetCurrentProcess();

		MODULEINFO brainstorm_info;
		GetModuleInformation(process, brainstorm, &brainstorm_info, sizeof(MODULEINFO));

		FIND_OFFSET(INTEGRITY_CHECK_PATTERN, integrity_check_offset, brainstorm_info);
	}
	...
	void Initialize() {
		...
		INITIALIZE_OFFSET(METHOD_FROM_INTERPRETED_ENTRY_PATTERN, Method_from_interpreted_entry_offset, DWORD, 4);
...
public:
	...
	inline PVOID* from_interpreted_entry_p() {
		return (PVOID*)((BYTE*)this + Offsets::Method_from_interpreted_entry_offset);
	}
};
